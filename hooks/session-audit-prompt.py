...
Registered in ``hooks/hooks.json`` and ships with the plugin.
    ...
    try:
        with open(path, encoding="utf-8") as fh:
            for lineno, raw_line in enumerate(fh, start=1):
                ...
                # Claude Code wraps messages under an "message" key:
                #   {"parentUuid": ..., "message": {"role": ..., "content": ...}}
                # Fall back to the flat top-level shape for legacy transcripts
                # and unit-test fixtures that use the unwrapped format.
                msg = (
                    event["message"]
